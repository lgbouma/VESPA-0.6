...
from .populations import (
    PopulationSet, DEFAULT_MODELS, ArtificialPopulation, NO_EB_OR_HEBS
)
...
class FPPCalculation(object):
    ...
    * Reading from a config file, using :func:`FPPCalculation.from_ini`
    ...
    def FPPhack(self,fig=None,figsize=(10,8),saveplot=False,folder='.',
                   starinfo=True,siginfo=True,
                   priorinfo=True,constraintinfo=True,
                   tag=None,simple=False,figformat='png',skipmodels=None):
        """
        Makes FPP summary plot with certain populations dropped.
        ...
        for model in self.popset.modelnames:
            if model not in skipmodels:
                ...
                lhoods.append(self.lhood(model))
                if np.isnan(priors[-1]):
                    raise ValueError('{} prior is nan; priorfactors={}'.
                                     format(model, self.priorfactors))
                Ls.append(priors[-1]*lhoods[-1])
        ...
        Ls = np.array(Ls)
        logging.debug('modelnames={}'.format(modelnames))
        logging.debug('priors={}'.format(priors))
        logging.debug('lhoods={}'.format(lhoods))

        nmodels = len(self.popset.modelnames) - len(skipmodels)
        colors = [cm.jet(1.*i/nmodels) for i in range(nmodels)]
        legendprop = {'size':8}
        ...
        if len(skipmodels) > 0:
            for m in skipmodels:
                constraints += '\n Exclude {}'.format(str(m))

        if constraintinfo:
            plt.annotate(constraints,xy=(0.03,0.22),xycoords='figure fraction',
                         va='top',color='red')

        odds = 1./self.FPP(skipmodels=skipmodels)
        ...
        plt.annotate('$f_{pl,V} = %.3f$\n%s' % (self.fpV(skipmodels=skipmodels),fppstr),
                     xy=(0.7,0.02), xycoords='figure fraction', fontsize=16,
                     va='bottom')
        ...
        if saveplot:
            if tag is not None:
                plt.savefig('%s/FPPhack%s.%s' % (folder,tag,figformat))
            else:
                plt.savefig('%s/FPPhack.%s' % (folder,figformat))
            plt.close()
